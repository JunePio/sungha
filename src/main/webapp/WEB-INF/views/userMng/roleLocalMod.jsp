...
<script type="text/javascript">
	var localIdArrayMod = new Array(); // 전역변수 지역ID 저장
	var localArrayMod = new Array(); // 전역변수 지역명 저장
	...
	function fnCheckChange(localId, local) {
		...
		if($('#'+localId).is(":checked")) { // 체크되었을 경우
			localIdArrayMod.push(localId); // 지역ID 저장
			localArrayMod.push(local); // 지역명 저장
		} else { // 체크해제 되었을 경우
			
			for(var i = 0; i < localIdArrayMod.length; i++) {
				if(localIdArrayMod[i] == localId) {
					localIdArrayMod.splice(i, 1); // 체크 해제된 지역ID 제거
					localArrayMod.splice(i, 1); // 체크 해제된 지역명 제거
					break;
				}
			}
		}
	...
	// 확인 버튼 클릭시
	function fnSend() {
		var roleLocalIdMod = window.opener.document.getElementById("roleLocalIdMod"); // 부모창에 지역ID 넘겨주기
		var roleLocalMod = window.opener.document.getElementById("roleLocalMod"); // 부모창에 지역명 넘겨주기
		roleLocalIdMod.value = localIdArrayMod;
		roleLocalMod.value = localArrayMod;
		window.close();
		
	};
	
	function fnClose() {
		localIdArrayMod = new Array();
		localArrayMod = new Array();
		window.close();
	}
...
<body>
	...
	<c:otherwise>
		<input type="checkbox" id="totalCheck" name="eachCheck" onclick="fnTotalCheckChange(this)"><c:out value="모두선택" />
		<c:forEach var="item" items="${roleLocalList}" varStatus="status">
		<tbody id="tbody">
			<tr align="center">
				<td id="check"><input type="checkbox" id="${item.localId}" name="eachCheck" value="${item.local}" onchange="fnCheckChange('${item.localId}', '${item.local}')"><span id="${item.localId}">${item.local}</span></td>
			</tr>
		</tbody>
		</c:forEach>
	</c:otherwise>
	...
	<input type="button" id="confirm" name="confirm" value="확인" onclick="fnSend()"><input type="button" id="cancel" name="cancel" value="취소" onclick="fnClose()">
	</form>
</body>
</html>