 ...
 * @Description : 공지사항 등록 화면
 * @Modification Information
 * @ 수정일                  수정자           수정내용
 * @ ----------  -------  -------------------------------
 * @ 2023.02.10  이창호      최초생성
 * @ 2023.05.03  이준영      전반적인 css 추가, 수정, 메뉴, 푸터 추가
 */
%>
...
<head>
	...
	function fnSave(){
		...
		var noticeGubun = $("#noticeGubun").val();
		...
		if(writerId == "") {
			...
			return false;
		} else if(noticeGubun == " ") {  // 저장시 아이디 중복체크를 했는지 다시 체크
			alert("문의종류 확인 해주세요.");
			return false;
		}
		...
		$.ajax({
			...
			if(data.saveResult) {
				alert("저장완료 하였습니다.");
				document.regForm.action = '<c:url value="/notice/noticeList.do"/>'; // 전송 url & 제이쿼리로 하면 action이 동작되지 않아서 DOM Script로 하였습니다.
			    $("#regForm").submit();
...
<div class="sh-app">
	...
	<div class="sh-main">
		<div class="sh-header">
			<div class="title">
				<p class="title-txt">공지사항 등록
					<span class="title-menu"> < 공지사항 < 등록</span>
				</p>
			</div>
		</div>
		<div class="sh-content">
			<div class="main-content" headline="Stats">
					<form id="regForm" name="regForm" action="/notice/noticeSave.do" method="post">
							...
							<div class="reg-info-agency" >
								...
								<div class="wrap-table-detail-agency">
										...
										<tr>
											...
											<td class="reg-select-td">
												<div class="res-reg-td">
													...
													<select id="noticeGubun" name="noticeGubun" class="dropdown-sel reg-select res-sel">
											...
											<td class="reg-input1">
												<div class="res-reg-td">
													...
													<div class="agency-loc-area">
														<textarea class="textarea-reg-1q"  id="textArea_byteLimit" name="textArea_byteLimit" onkeydown="fnCheckByte(event, this)" onkeyup="fnCheckByte(event, this)"></textarea>
													</div>
									...
									<div class="btn-area-reg-detail" float="center">
										<button type="submit"  class="list-btn-agency-type2" id="user-reg-btn"><span  onclick="fnSave()">저장</span></button>
										<button type="button" id="modal_close_btn"  class="list-btn-agency-type2"><span onclick="fnCloseRegModal()">취소</span></button>
